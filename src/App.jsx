...
const STORAGE_KEY = "goweb-prototype-state-v13";
const LEGACY_STORAGE_KEY = "goweb-prototype-state-v12";
...
function Wishlist({wishlists,setWishlists,mergedLists,setMergedLists,mergedContents,setMergedContents,onNavigate,onCart,onRemoveWishlistItem}) {
  ...
  return <main className="page"><div className="pageTitle split"><div><span className="eyebrow">YOUR LISTS</span><h1>Wishlist</h1></div><button className="primary small" onClick={()=>setMergeOpen(true)}>Merge List</button></div>
    <div className="wishColumns">{["A","B","C"].map(k=><section className="wishCol" key={k}><h3>Wishlist {k}</h3><div className="wishItems">{(wishlists[k]||[]).length?wishlists[k].map(id=>{const p=products.find(x=>x.id===id);if(!p)return null;return <WishlistItem key={id} p={p} onCart={onCart} onRemove={id=>onRemoveWishlistItem(k,id)}/>;}):<p className="empty">No products in this list.</p>}</div></section>)}</div>
...
function WishlistItem({p,onCart,onRemove}){
  const [quantity,setQuantity]=useState(1);
  const add=()=>{onRemove(p.id);onCart(p,quantity,true)};
  return <div className="wishItem"><img src={p.image} alt={p.name}/><div className="wishItemMain"><b>{p.name}</b><span>₹{p.price.toLocaleString("en-IN")}</span><div className="itemActions"><QuantitySelect value={quantity} onChange={setQuantity}/><button className="listActionBtn" onClick={add}>Add to Cart</button><button className="deleteBtn listDelete" onClick={()=>onRemove(p.id)}>Delete</button></div></div></div>;
}
...
export default function App(){
  ...
  const removeWishlistProduct=(listKey,productId)=>{
    const id=String(productId);
    setWishlists(w=>({...w,[listKey]:(w[listKey]||[]).filter(x=>String(x)!==id)}));

    setMergedContents(current=>{
      const next={...current};
      const emptyKeys=[];
      Object.keys(next).forEach(key=>{
        if(!key.includes(listKey))return;
        const remaining=(Array.isArray(next[key])?next[key]:[]).filter(x=>String(x)!==id);
        if(remaining.length)next[key]=remaining;
        else{delete next[key];emptyKeys.push(key);}
      });
      if(emptyKeys.length)setMergedLists(current=>current.filter(key=>!emptyKeys.includes(key)));
      return next;
    });
  };
  ...
  return <div className="app">
    ...
    {view==="wishlist"&&<Wishlist onRemoveWishlistItem={removeWishlistProduct} wishlists={wishlists} setWishlists={setWishlists} mergedLists={mergedLists} setMergedLists={setMergedLists} mergedContents={mergedContents} setMergedContents={setMergedContents} onNavigate={navigate} onCart={addCart}/>} 
